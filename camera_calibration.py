...
if __name__ == "__main__":
    ...
    parser.add_argument("--calibration-images", default=os.path.join(script_dir, "calibration_images"), help="dir with images for calibration")
    parser.add_argument("--save-dir", default=os.path.join(script_dir, "camera_params"), help="dir with camera calibration parameters")
    parser.add_argument("--dry-run", action="store_true", help="Do not write new calibration params in --save-dir")
    ...
    if not opt.dry_run:
        ...
        for k, v in save_names.items():
            np.save(os.path.join(args.save_dir, k), v)
