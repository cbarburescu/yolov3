...
SEQUENTIAL_LAYERS = [nn.Sequential, nn.ModuleList]
...
class Histogrammer(nn.Module):
    def __init__(
        self,
        histogrammer
    ):
        super(Histogrammer, self).__init__()
        self.histogram = histogrammer

    def forward(self, x):
        pdb.set_trace()
        return self.histogram("NAME", x)


def _get_return_sequential_plot_func(hist, layer_types=[], except_layers=[]):
    for x in except_layers:
        assert isinstance(x, int)

    def _insert_hist_layer(module):
        """Insert histogrammer layer for all layers so long as in layer_types
        """

        if type(module) in SEQUENTIAL_LAYERS:
            for i, sub_module in enumerate(module.children()):
                if i not in except_layers:
                    module[i] = _insert_hist_layer(module[i])
            return module
        elif type(module) in DICT_LAYERS:
            for key, sub_module in module.items():
                module[key] = _insert_hist_layer(module[key])
            return module
            ...
            if type(module) in lp_layer_types:
                module = nn.Sequential(module, hist)
            return module

    return _insert_hist_layer


def sequential_plot_activations(
    model,
    histogrammer,
    layer_types=[],
    except_layers=[],
    ...
    histogram_func = _get_return_sequential_plot_func(
        hist, layer_types=layer_types, except_layers=except_layers)
    return histogram_func(copy.deepcopy(model))
